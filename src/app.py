    ...
    for note in notes:
        ...
        if color == "crimson":
            ...
            if non_red_count == 1:
                ...
                feedback.append("You're not very good at this... Please just stick to crimson.")
    
    combined = list(zip(notes, feedback))
    selected_color = notes[-1].name if notes else "AntiqueWhite"
    return render_template("index.html", color_options=color_options, color_feedback=combined, selected_color=selected_color)
